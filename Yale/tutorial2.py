### MAKE CONNECTION between NEURON
...
# NetCon : communation tool (source to target signaling)
# NetStim : spike generator (typically source)

stim = h.NetStim()  # Make a new stimulator

# Attach it to a synapse in the middle of the dendrite of the first cell in the network.
# (Named 'syn_' to avoid being overwritten with the 'syn' var assigned later.)
syn_ = h.ExpSyn(my_cells[0].dend(0.5)) # AMPA synapse role - current decaying exponentaially
syn_.tau = 2 * ms
...
ncstim.delay = 1 * ms # 1초 뒤 (10초 때) synapse에 spike 전달
ncstim.weight[0] = 0.04  # NetCon weight is a vector.

# ========================================================================
# Stimulation - plotting

# plotting - single neuron
def plotting_single_neuron():
...
# plotting - syanaptic conductance
def plotting_synaptic_conductance():
    recording_cell = my_cells[0] # only react in 0-th neuron
    soma_v = h.Vector().record(recording_cell.soma(0.5)._ref_v)
    dend_v = h.Vector().record(recording_cell.dend(0.5)._ref_v)
    t = h.Vector().record(h._ref_t)
...
recording_cell = my_cells[1] # only react in 0-th neuron
soma_v = h.Vector().record(recording_cell.soma(0.5)._ref_v)
dend_v = h.Vector().record(recording_cell.dend(0.5)._ref_v)
t = h.Vector().record(h._ref_t)
...
# plotting - 0-th cell firing
plt.plot(t, soma_v, label="soma(0.5)")
plt.plot(t, dend_v, label="dend(0.5)")
plt.legend()
plt.show()

# plotting - all cell spikes
